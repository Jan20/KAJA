...
def fetch_data_dir(directory, limit):
    ...
    replay_files = sorted([f for f in os.listdir(directory) if
                           os.path.isfile(os.path.join(directory, f)) and (f.startswith("2Player-halite-2-gold"))])
...
def main():
    ...
    nn = NeuralNetTwoplayer(cached_model=args.cache, seed=args.seed)
