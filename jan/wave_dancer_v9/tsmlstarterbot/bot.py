...
from tsmlstarterbot.neural_net_4player import NeuralNetMultiplayer
from tsmlstarterbot.neural_net_2player import NeuralNetTwoplayer
...
class Bot:
    def __init__(self, location4,location2, name):
        current_directory = os.path.dirname(os.path.abspath(__file__))
        self._model_location4 = os.path.join(current_directory, os.path.pardir, "models", location4)
        self._model_location2 = os.path.join(current_directory, os.path.pardir, "models", location2)
        self._name = name
        self._neural_net = NeuralNetMultiplayer(cached_model=self._model_location4)
        self._neural_net_two = NeuralNetTwoplayer(cached_model=self._model_location2)
        ...
        predictions2 = self._neural_net_two.predict(random_input_data)
    ...
    def play(self):
        ...
        while True:
            ...
            if(len(game_map.all_players())==2):
                self.neural_net = self._neural_net_two
                logging.info("NeuralNet for 2 selected")
    ...
    def produce_instructions(self, game_map, ships_to_planets_assignment, round_start_time):
        ...
        for ship, planet in ships_to_planets_assignment:
            ...
            ############################
            elif closest_enemy_ship_distance < 7 and ship.health > 64 and ship.health < 129 and ship.health < closest_enemy_ship.health and closest_enemy_ship.health > 64:
                logging.info("Himmelfahrtskommando")
                command_queue.append(
                    self.navigate(game_map, round_start_time, ship, closest_enemy_ship, speed))

            ###################################
            ### End of Himmelfahrtskommando ###
            ###################################
            ...
            #######################################
                ...
                if closest_team_ship != None:
                    command_queue.append(
                        self.navigate(game_map, round_start_time, ship, ship.closest_point_to(closest_team_ship), speed))

            ##############################################
            ### End of Unus pro omnibus, omnes pro uno ###
            ##############################################

            ###################################
            ### Mining Ship Attack Mechanic ###
            ###################################
            elif closest_enemy_ship_distance < 20 and closest_enemy_ship.DockingStatus.DOCKED:
            ...
            ##########################################
            elif closest_enemy_ship_distance < 11:
                command_queue.append(
                    self.navigate(game_map, round_start_time, ship, ship.closest_point_to(closest_enemy_ship), speed))
            ...
            ### Combat Mechanic ###
            #######################
            elif closest_enemy_ship_distance < 10:

                if closest_team_ship_distance > 5:
                    logging.info('Combat Mechanic: Retriet')
                    if closest_team_ship != None:
                    ...
                    logging.info('Combat Mechanic: Attack')
