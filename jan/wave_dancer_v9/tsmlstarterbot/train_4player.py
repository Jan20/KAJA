...
def fetch_data_dir(directory, limit):
    ...
    replay_files = sorted([f for f in os.listdir(directory) if
                           os.path.isfile(os.path.join(directory, f)) and (f.startswith("4Player"))])
    ...
    for r in replay_files:
        ...
        with open(full_path) as game:
            ...
            print("opened file "+full_path+" Nr. "+str(loaded_games))
            game_data = game.read()
...
def main():
    ...
    nn = NeuralNetMultiplayer(cached_model=args.cache, seed=args.seed)
